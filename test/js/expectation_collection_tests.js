...
jsMochaTests.ExpectationCollectionTests = function(Y) {
	...
	testSuite.add(new Y.Test.Case({
		...
		setUp : function () {
			this.expectation_collection = new jsMocha.ExpectationCollection();
			this.mock = new Mock();
		...
		tearDown : function () { 
			delete this.expectation_collection;
			delete this.mock;
		...
		testHasArray : function () {
			Y.Assert.isArray(this.expectation_collection.expectations);
		},
		
		testCanAddExpectations : function () {
			this.expectation_collection.add(this.expectation1);
			Y.Assert.areEqual(1, this.expectation_collection.expectations.length);
			
			this.expectation_collection.add(this.expectation2);
			Y.Assert.areEqual(2, this.expectation_collection.expectations.length);
		},
