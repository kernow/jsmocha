...
jsMocha.ParametersMatcher.prototype = {
    ...
    if(parameters){
         ...
         for(var i=0; i<parameters.length; i++){
           a.push(this.serialize(parameters[i], true));
         }
         return a.join(', ');
        }
	},
	serialize: function(obj, first_call) {
	  if(first_call){this.serialize_stack_calls = 0;}
	  if(this.serialize_stack_calls > this.serialize_stack_limit){
	    return 'object too complex to fully display';
    }
    this.serialize_stack_calls += 1;
          ...
          if (obj.constructor === Array || typeof obj.callee !== 'undefined'){
            ...
            for(var key in obj) {
              if (obj.hasOwnProperty(key)) {
                a.push(key + ':' + this.serialize(obj[key]));
              }
            }
            str = '{' + a.join(', ').replace(/\,$/, '') + '}';
          }
