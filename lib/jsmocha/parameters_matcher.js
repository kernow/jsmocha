...
ParametersMatcher.prototype = {
	...
	match: function(actual_parameters) {
		...
		if(this.block_given()){
		  return this.block(actual_parameters);
		}
		else{
    ...
    // console.log(this.expected_parameters.length);
	  if(this.expected_parameters.length == 1 && this.is_function(this.expected_parameters)){
	    this.block = this.expected_parameters;
	    return true;
	  }
	  else{
	    return false;
