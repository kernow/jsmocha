...
jsMocha.ExpectationList.prototype = {
	...
	add: function(mock, method_name){
	  ...
	  this.find_or_create(method_name).push(expectation);
		return expectation;
	},
	...
	verify_all: function(){
		...
		for(var key in this.expectations){
		  if (this.expectations.hasOwnProperty(key)) {
  		  var len = this.expectations[key].length;
    		for(var i = 0; i < len; i++){
    			results.push(this.expectations[key][i].verify());
    		}
  		}
		}
	...
	reports: function(){
		...
		for(var key in this.expectations){
		  if (this.expectations.hasOwnProperty(key)) {
  		  var len = this.expectations[key].length;
    		for(var i = 0; i < len; i++){
    			results.push(this.expectations[key][i].report());
    		}
  		}
		}
		return "\r\n"+results.join("\r\n");
	...
	restore_all: function(){
	  for(var key in this.expectations){
		  if (this.expectations.hasOwnProperty(key)) {
  		  var len = this.expectations[key].length;
    		for(var i = 0; i < len; i++){
    			this.expectations[key][i].restore();
    		}
  		}
		}
	}
