...
function Expectation(mock, method_name, stub) {
	this.mock = mock;
...
Expectation.prototype = {
	...
	verify: function(){
		...
		if(this.invocation_count != this.expected_invocations || this.expected_invocations == 'any'){
			this.set_valid(false);
			this.add_report('FAIL wrong number of invocations, expected '+this.expected_invocations+' invoked '+this.invocation_count+' times');
		}
		else{
			this.set_valid(true);
			this.add_report('PASS invoked '+this.invocation_count+' times');
		}
